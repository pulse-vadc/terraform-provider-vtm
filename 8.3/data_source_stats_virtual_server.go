...
func dataSourceVirtualServerStatistics() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			// UDP bytes received by this virtual server from clients but failed
			//  to be sent to backend servers.
			"udp_bytes_in_dropped": &schema.Schema{
				Type:     schema.TypeInt,
				Optional: true,
			},

			// UDP bytes received by this virtual server from backend servers
			//  but failed to be sent to clients.
			"udp_bytes_out_dropped": &schema.Schema{
				Type:     schema.TypeInt,
				Optional: true,
			},
...
func dataSourceVirtualServerStatisticsRead(d *schema.ResourceData, tm interface{}) (readError error) {
	...
	lastAssignedField = "udp_bytes_in_dropped"
	d.Set("udp_bytes_in_dropped", int(*object.Statistics.UdpBytesInDropped))

	lastAssignedField = "udp_bytes_out_dropped"
	d.Set("udp_bytes_out_dropped", int(*object.Statistics.UdpBytesOutDropped))
