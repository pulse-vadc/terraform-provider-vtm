...
func dataSourceGlobalsStatistics() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			// UDP bytes received by the traffic manager from clients but failed
			//  to be sent to backend servers.
			"total_udp_bytes_in_dropped": &schema.Schema{
				Type:     schema.TypeInt,
				Optional: true,
			},

			// UDP bytes received by the traffic manager from backend servers
			//  but failed to be sent to clients.
			"total_udp_bytes_out_dropped": &schema.Schema{
				Type:     schema.TypeInt,
				Optional: true,
			},
...
func dataSourceGlobalsStatisticsRead(d *schema.ResourceData, tm interface{}) (readError error) {
	...
	lastAssignedField = "total_udp_bytes_in_dropped"
	d.Set("total_udp_bytes_in_dropped", int(*object.Statistics.TotalUdpBytesInDropped))

	lastAssignedField = "total_udp_bytes_out_dropped"
	d.Set("total_udp_bytes_out_dropped", int(*object.Statistics.TotalUdpBytesOutDropped))
