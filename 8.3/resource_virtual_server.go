...
func getResourceVirtualServerSchema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"protocol": &schema.Schema{
			Type:         schema.TypeString,
			Optional:     true,
			ValidateFunc: validation.StringInSlice([]string{"client_first", "dns", "dns_tcp", "ftp", "http", "https", "imaps", "imapv2", "imapv3", "imapv4", "ldap", "ldaps", "pop3", "pop3s", "rtsp", "server_first", "siptcp", "sipudp", "smtp", "ssl", "stream", "telnet", "udp", "udpstreaming"}, false),
			Default:      "http",
		},
		...
		//  use to store data sent by the client through one TCP connection
		//  or HTTP/2 stream.  Larger values will use more memory, but will
		//  minimise the number of "read()" and "write()" system calls that
		//  the traffic manager must perform.
		...
		// The amount of memory, in bytes, that the virtual server should
		//  use to store data returned by the server through one TCP connection.
		//   Larger values will use more memory, but will minimise the number
		//  of "read()" and "write()" system calls that the traffic manager
		//  must perform.
		...
		//  "max_client_buffer", that the virtual server should use to store
		//  data sent by a client through a HTTP/2 connection. The value
		//  specified can be between 0 and 200. The value of "0" means unlimited.
		//  This setting limits buffer size for a HTTP/2 connection and does
		//  not affect buffer size for HTTP/1 connections or TCP stream connections.
		//  The number of HTTP/2 streams that can be opened in a single HTTP/2
		//  connection is given by the "http2!max_concurrent_streams". An
		//  overall cap to the amount of memory allocated for buffers for
		//  all TCP connections is given by the global "max_tcp_buff_mem"
		//  setting.
		"http2_http2_client_buffer_multiplier": &schema.Schema{
			Type:         schema.TypeInt,
			Optional:     true,
		...
		// The amount of memory, in multiples of the value specified by
		//  "max_server_buffer", that the virtual server should use to store
		//  data sent to a client through HTTP/2 connection. The value specified
		//  can be between 0 and 200. The value of "0" means unlimited. This
		//  setting limits buffer size for a HTTP/2 connection and does not
		//  affect buffer size for HTTP/1 connections or TCP stream connections.
		//  The number of HTTP/2 streams that can be opened in a single HTTP/2
		//  connection is given by the "http2!max_concurrent_streams". An
		//  overall cap to the amount of memory allocated for buffers for
		//  all TCP connections is given by the global "max_tcp_buff_mem"
		//  setting.
		"http2_http2_server_buffer_multiplier": &schema.Schema{
			Type:         schema.TypeInt,
			Optional:     true,
			ValidateFunc: validation.IntBetween(0, 200),
			Default:      0,
		...
		"sip_udp_associate_by_source": &schema.Schema{
			Type:     schema.TypeBool,
			Optional: true,
			Default:  true,
		},
		...
		//  traffic manager processes, if this behaviour is not normally
		//  selected automatically due to other settings.
		...
		// If this setting is non-zero, the virtual server will set the
		//  socket receive buffer size to this number of bytes. If set, this
		//  will override the <a href="?fold_open=System Tuneables&section=Global
		//  Settings">so_rbuff_size</a> setting. An OS-specified limit on
		//  socket buffer sizes such as given by "sysctl net.core.rmem_max"
		//  can be exceeded using this setting.
		"udp_rbuff_size": &schema.Schema{
			Type:         schema.TypeInt,
			Optional:     true,
		...
		// Whether the traffic manager should try to use SO_REUSEPORT for
		//  distributing incoming UDP datagrams across multiple processes
		//  (if kernel support is detected) or whether the legacy (pre-20.2)
		//  multi-processing mode should be used.
		"udp_smp_mode": &schema.Schema{
			Type:         schema.TypeString,
			Optional:     true,
			ValidateFunc: validation.StringInSlice([]string{"auto", "legacy"}, false),
			Default:      "auto",
		},
		...
		// If this setting is non-zero, the virtual server will set the
		//  socket send buffer size to this number of bytes. If set, this
		//  will override the <a href="?fold_open=System Tuneables&section=Global
		//  Settings">so_wbuff_size</a> setting. An OS-specified limit on
		//  socket buffer sizes such as given by "sysctl net.core.wmem_max"
		//  can be exceeded using this setting.
		"udp_wbuff_size": &schema.Schema{
			Type:         schema.TypeInt,
			Optional:     true,
			ValidateFunc: validation.IntBetween(0, 20000000),
			Default:      0,
...
func resourceVirtualServerRead(d *schema.ResourceData, tm interface{}) (readError error) {
	...
	lastAssignedField = "http2_http2_client_buffer_multiplier"
	d.Set("http2_http2_client_buffer_multiplier", int(*object.Http2.Http2ClientBufferMultiplier))
	lastAssignedField = "http2_http2_server_buffer_multiplier"
	d.Set("http2_http2_server_buffer_multiplier", int(*object.Http2.Http2ServerBufferMultiplier))
	...
	lastAssignedField = "sip_udp_associate_by_source"
	d.Set("sip_udp_associate_by_source", bool(*object.Sip.UdpAssociateBySource))
	...
	lastAssignedField = "ssl_ca_sites"
	sslCaSites := make([]map[string]interface{}, 0, len(*object.Ssl.CaSites))
	for _, item := range *object.Ssl.CaSites {
		itemTerraform := make(map[string]interface{})
		if item.CertHeaders != nil {
			itemTerraform["cert_headers"] = string(*item.CertHeaders)
		}
		if item.ClientCas != nil {
			itemTerraform["client_cas"] = []string(*item.ClientCas)
		}
		if item.Host != nil {
			itemTerraform["host"] = string(*item.Host)
		}
		if item.RequestCert != nil {
			itemTerraform["request_cert"] = string(*item.RequestCert)
		}
		sslCaSites = append(sslCaSites, itemTerraform)
	}
	d.Set("ssl_ca_sites", sslCaSites)
	sslCaSitesJson, _ := json.Marshal(sslCaSites)
	d.Set("ssl_ca_sites_json", sslCaSitesJson)
	...
	d.Set("ssl_support_tls1_3", string(*object.Ssl.SupportTls13))
	...
	lastAssignedField = "udp_rbuff_size"
	d.Set("udp_rbuff_size", int(*object.Udp.RbuffSize))
	...
	lastAssignedField = "udp_smp_mode"
	d.Set("udp_smp_mode", string(*object.Udp.SmpMode))
	lastAssignedField = "udp_timeout"
	d.Set("udp_timeout", int(*object.Udp.Timeout))
	lastAssignedField = "udp_wbuff_size"
	d.Set("udp_wbuff_size", int(*object.Udp.WbuffSize))
...
func resourceVirtualServerObjectFieldAssignments(d *schema.ResourceData, object *vtm.VirtualServer) {
	...
	setInt(&object.Http2.Http2ClientBufferMultiplier, d, "http2_http2_client_buffer_multiplier")
	setInt(&object.Http2.Http2ServerBufferMultiplier, d, "http2_http2_server_buffer_multiplier")
	...
	setBool(&object.Sip.UdpAssociateBySource, d, "sip_udp_associate_by_source")
	setBool(&object.Smtp.ExpectStarttls, d, "smtp_expect_starttls")
	setBool(&object.Ssl.AddHttpHeaders, d, "ssl_add_http_headers")

	object.Ssl.CaSites = &vtm.VirtualServerCaSitesTable{}
	if sslCaSitesJson, ok := d.GetOk("ssl_ca_sites_json"); ok {
		_ = json.Unmarshal([]byte(sslCaSitesJson.(string)), object.Ssl.CaSites)
	} else if sslCaSites, ok := d.GetOk("ssl_ca_sites"); ok {
		for _, row := range sslCaSites.(*schema.Set).List() {
			itemTerraform := row.(map[string]interface{})
			VtmObject := vtm.VirtualServerCaSites{}
			VtmObject.CertHeaders = getStringAddr(itemTerraform["cert_headers"].(string))
			VtmObject.ClientCas = getStringSetAddr(expandStringSet(itemTerraform["client_cas"].(*schema.Set)))
			VtmObject.Host = getStringAddr(itemTerraform["host"].(string))
			VtmObject.RequestCert = getStringAddr(itemTerraform["request_cert"].(string))
			*object.Ssl.CaSites = append(*object.Ssl.CaSites, VtmObject)
		}
		d.Set("ssl_ca_sites", sslCaSites)
	} else {
		d.Set("ssl_ca_sites", make([]map[string]interface{}, 0, len(*object.Ssl.CaSites)))
	}
	...
	setString(&object.Ssl.SupportTls13, d, "ssl_support_tls1_3")
	...
	setInt(&object.Udp.RbuffSize, d, "udp_rbuff_size")
	setInt(&object.Udp.ResponseDatagramsExpected, d, "udp_response_datagrams_expected")
	setString(&object.Udp.SmpMode, d, "udp_smp_mode")
	setInt(&object.Udp.Timeout, d, "udp_timeout")
	setInt(&object.Udp.WbuffSize, d, "udp_wbuff_size")
