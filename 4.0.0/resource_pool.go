...
func resourcePoolCreate(d *schema.ResourceData, tm interface{}) error {
	...
	if _, ok := d.GetOk("monitors"); ok {
		setStringList(&object.Basic.Monitors, d, "monitors")
	} else {
	...
	if _, ok := d.GetOk("auto_scaling_securitygroupids"); ok {
		setStringList(&object.AutoScaling.Securitygroupids, d, "auto_scaling_securitygroupids")
	} else {
	...
	if _, ok := d.GetOk("auto_scaling_subnetids"); ok {
		setStringList(&object.AutoScaling.Subnetids, d, "auto_scaling_subnetids")
	} else {
	...
	if nodesTableJson, ok := d.GetOk("nodes_table_json"); ok {
		...
		for _, row := range nodesTable.(*schema.Set).List() { // VTM-37687: nodesTable.([]interface{}) {
	...
	if _, ok := d.GetOk("dns_autoscale_hostnames"); ok {
		setStringList(&object.DnsAutoscale.Hostnames, d, "dns_autoscale_hostnames")
	} else {
	...
	setBool(&object.L4Accel.Snat, d, "l4accel_snat")
	...
	if _, ok := d.GetOk("ssl_common_name_match"); ok {
		setStringList(&object.Ssl.CommonNameMatch, d, "ssl_common_name_match")
	} else {
