...
func dataSourceCacheUniSessionCacheStatistics() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			// Number of times an expired universal session entry has been dropped
			//  on lookup.
			"expiries": &schema.Schema{
				Type:     schema.TypeInt,
				Optional: true,
			},
...
func dataSourceCacheUniSessionCacheStatisticsRead(d *schema.ResourceData, tm interface{}) (readError error) {
	...
	lastAssignedField = "expiries"
	d.Set("expiries", int(*object.Statistics.Expiries))
