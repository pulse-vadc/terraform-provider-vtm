...
func getResourceUserAuthenticatorSchema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		// The type of TLS encryption, if any, to use. Usually STARTTLS
		//  will be used with port 389, and LDAPS with port 636. A Certificate
		//  Authority that the LDAP server's certificate chains back to must
		//  be present in the "Admin Certificate Authorities and Certificate
		//  Revocation Lists Catalog" under "SSL catalogs", otherwise the
		//  connection will fail.
		"ldap_ssl": &schema.Schema{
			Type:         schema.TypeString,
			Optional:     true,
			ValidateFunc: validation.StringInSlice([]string{"ldaps", "none", "starttls"}, false),
			Default:      "none",
		},
...
func resourceUserAuthenticatorRead(d *schema.ResourceData, tm interface{}) (readError error) {
	...
	lastAssignedField = "ldap_ssl"
	d.Set("ldap_ssl", string(*object.Ldap.Ssl))
...
func resourceUserAuthenticatorObjectFieldAssignments(d *schema.ResourceData, object *vtm.UserAuthenticator) {
	...
	setString(&object.Ldap.Ssl, d, "ldap_ssl")
