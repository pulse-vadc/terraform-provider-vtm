...
func (vtm VirtualTrafficManager) GetTrafficManager(name string) (*TrafficManager, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/8.3/config/active/traffic_managers/" + name + "?expert_keys=basic/community_edition_accepted")
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) NewTrafficManager(name string) *TrafficManager {
	...
	conn := vtm.connector.getChildConnector("/tm/8.3/config/active/traffic_managers/" + name)
	object.connector = conn
	return object
}
...
func (vtm VirtualTrafficManager) ListTrafficManagers() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/8.3/config/active/traffic_managers")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
...
type TrafficManagerProperties struct {
	Admin struct {
		...
		HstsEnable *bool `json:"hsts_enable,omitempty"`

		// The number of seconds that the HSTS header field max-age will
		//  be set to
	...
	Iptrans struct {
		...
		//  requires the iptables socket extension.
