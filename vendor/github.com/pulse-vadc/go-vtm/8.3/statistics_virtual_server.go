...
type VirtualServerStatistics struct {
	Statistics struct {
		...
		UdpBytesInDropped         *int    `json:"udp_bytes_in_dropped"`
		UdpBytesOutDropped        *int    `json:"udp_bytes_out_dropped"`
...
func (vtm VirtualTrafficManager) GetVirtualServerStatistics(name string) (*VirtualServerStatistics, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/8.3/status/local_tm/statistics/virtual_servers/" + name)
	data, ok := conn.get()
	if ok != true {
