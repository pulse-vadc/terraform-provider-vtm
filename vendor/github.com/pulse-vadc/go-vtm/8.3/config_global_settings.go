...
func (vtm VirtualTrafficManager) GetGlobalSettings() (*GlobalSettings, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/8.3/config/active/global_settings?expert_keys=fault_tolerance/multicast_version,telemetry/autotest_schedule")
	data, ok := conn.get()
	if ok != true {
...
type GlobalSettingsProperties struct {
	...
	Basic struct {
		...
		// Maximum number of queued HTTP/2 frames per connection before
		//  the connection is closed. Intended to protect against CVE-2019-9514
		//  and similar exploits.
		Http2ConnFrameQueueLimit *int `json:"http2_conn_frame_queue_limit,omitempty"`
		...
		// The maximum amount of memory allowed to be used to buffer network
		//  data in user space for all TCP connections. The TCP data buffered
		//  are either received from clients but before sending to pool nodes,
		//  or recevied from pool nodes but before sending to clients. This
		//  is specified as either a percentage of system RAM, "5%" for example,
		//  or an absolute size such as "1024MB" and "2GB". A numeric value
		//  without suffix MB, GB or % defaults to MB. A value of "800" means
		//  800MB. A value of "0" means unlimited.
		MaxTcpBuffMem *string `json:"max_tcp_buff_mem,omitempty"`
	...
	Connection struct {
		...
		// Whether or not the traffic manager should try to read multiple
		//  UDP packets from clients each time the kernel reports data received
		//  from clients. This can improve performance for the situation
		//  with high UDP traffic throughput from clients to the traffic
		//  manager. Therefore, in general it is recommended that this be
		//  set to 'Yes'.
		UdpReadMultiple *bool `json:"udp_read_multiple,omitempty"`
	} `json:"connection"`
	...
	Ip struct {
		// A table of MAC address/network interface to IP address mappings
		//  for each router where return path routing is required.
	...
	Session struct {
		...
		IpCacheExpiry *int `json:"ip_cache_expiry,omitempty"`
		...
		//  will not be reused if the time since it was last used exceeds
		//  this value. 0 indicates no expiry timeout.
		J2EeCacheExpiry *int `json:"j2ee_cache_expiry,omitempty"`
		...
		UniversalCacheExpiry *int `json:"universal_cache_expiry,omitempty"`
	...
	Telemetry struct {
		...
		// Allow the reporting of anonymized usage data for product improvement
...
type GlobalSettingsApplianceReturnpath struct {
	// The MAC address/network interface to IPv4 address mapping of
	//  a router the software is connected to. The value is the IPv4
	//  address, the "*" (asterisk) in the key name is the MAC address
	//  and an optional network interface name, for example, 00:50:56:a6:24:3d
	//  or 00:50:56:a6:24:3d#eth0.
	Ipv4 *string `json:"ipv4,omitempty"`

	// The MAC address/network interface to IPv6 address mapping of
	//  a router the software is connected to. The value is the IPv6
	//  address, the "*" (asterisk) in the key name is the MAC address
	//  and an optional network interface name, for example, 00:50:56:a6:24:3d
	//  or 00:50:56:a6:24:3d#eth0.
	Ipv6 *string `json:"ipv6,omitempty"`

	// The MAC address/network interface of a router the software is
	//  connected to.
