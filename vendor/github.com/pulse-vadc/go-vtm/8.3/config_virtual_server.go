...
func (vtm VirtualTrafficManager) GetVirtualServer(name string) (*VirtualServer, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/8.3/config/active/virtual_servers/" + name)
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) ListVirtualServers() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/8.3/config/active/virtual_servers")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
...
type VirtualServerProperties struct {
	...
	Http2 struct {
		...
		//  setting.
		Http2ClientBufferMultiplier *int `json:"http2_client_buffer_multiplier,omitempty"`
		...
		//  all TCP connections is given by the global "max_tcp_buff_mem"
		//  setting.
		Http2ServerBufferMultiplier *int `json:"http2_server_buffer_multiplier,omitempty"`
	...
	Ssl struct {
		...
		CaSites *VirtualServerCaSitesTable `json:"ca_sites,omitempty"`
	...
	Udp struct {
		...
		RbuffSize *int `json:"rbuff_size,omitempty"`
		...
		SmpMode *string `json:"smp_mode,omitempty"`
		...
		//  can be exceeded using this setting.
		WbuffSize *int `json:"wbuff_size,omitempty"`
	} `json:"udp"`
...
type VirtualServerCaSites struct {
	// Which parts of the client certificate, if any, should be inserted
	//  into requests to a back-end node, as header fields. The same
	//  fields as for "ssl_client_cert_headers" are made available, and
	//  optionally the base64 encoded certificate itself.
	CertHeaders *string `json:"cert_headers,omitempty"`

	// The certificate authorities used to verify client certificates
	//  for a particular destination site IP or SNI hostname. The specific
	//  site replaces the "*" (asterisk) in the key name, the value must
	//  be a valid file name in the "conf/ssl/cas" directory. The key
	//  can be specified multiple times to cover multiple IP addresses
	//  or SNI hostnames.
	ClientCas *[]string `json:"client_cas,omitempty"`

	// The 'host' key gives the hostname or IP destination address used
	//  to match incoming TLS connections to keys of table 'ca_sites'.
	//  The host can be a specific DNS name for use with the SNI extension,
	//  a specific destination IP address when no SNI matches, or either
	//  of those with wildcard */? characters.
	Host *string `json:"host,omitempty"`

	// Whether or not the virtual server should request an identifying
	//  certificate from each client connecting to particular destination
	//  IP address or SNI hostname. If a client certificate is requested
	...
	RequestCert *string `json:"request_cert,omitempty"`
}

type VirtualServerCaSitesTable []VirtualServerCaSites
