...
type GlobalsStatistics struct {
	Statistics struct {
		...
		TotalUdpBytesInDropped            *int `json:"total_udp_bytes_in_dropped"`
		TotalUdpBytesOutDropped           *int `json:"total_udp_bytes_out_dropped"`
		UpTime                            *int `json:"up_time"`
	} `json:"statistics"`
}

func (vtm VirtualTrafficManager) GetGlobalsStatistics() (*GlobalsStatistics, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/8.2/status/local_tm/statistics/globals")
	data, ok := conn.get()
	if ok != true {
