...
func (vtm VirtualTrafficManager) GetSslTicketKey(name string) (*SslTicketKey, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/7.1/config/active/ssl/ticket_keys/" + name)
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) ListSslTicketKeys() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/7.1/config/active/ssl/ticket_keys")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
