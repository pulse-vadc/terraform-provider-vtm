...
func (vtm VirtualTrafficManager) GetVirtualServer(name string) (*VirtualServer, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/4.0/config/active/virtual_servers/" + name)
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) ListVirtualServers() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/4.0/config/active/virtual_servers")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
...
type VirtualServerProperties struct {
	...
	Basic struct {
		...
		BypassDataPlaneAcceleration *bool `json:"bypass_data_plane_acceleration,omitempty"`
		...
		//  have ended.
		UdpEndTransaction *string `json:"udp_end_transaction,omitempty"`
	} `json:"basic"`
	...
	L4Accel struct {
		...
		RstOnServiceFailure *bool `json:"rst_on_service_failure,omitempty"`
		...
		ServiceIpSnat *bool `json:"service_ip_snat,omitempty"`
		...
		TcpMsl *int `json:"tcp_msl,omitempty"`
		...
		UdpCountRequests *bool `json:"udp_count_requests,omitempty"`
	} `json:"l4accel"`

	L4Stateless struct {
		// The initial ring size to use in the L4 stateless consistent hash
		//  table.
		InitialRingSize *int `json:"initial_ring_size,omitempty"`

		// The number of replicas per node to use in the L4 stateless consistent
		//  hash table.
