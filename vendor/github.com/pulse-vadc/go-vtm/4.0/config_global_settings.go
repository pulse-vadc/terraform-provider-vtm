...
func (vtm VirtualTrafficManager) GetGlobalSettings() (*GlobalSettings, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/4.0/config/active/global_settings")
	data, ok := conn.get()
	if ok != true {
...
type GlobalSettingsProperties struct {
	...
	Appliance struct {
		...
		//  automatically.
		ManageNcipher *bool `json:"manage_ncipher,omitempty"`
	...
	Ssl struct {
		...
		// The minimum length in bits of the Diffie-Hellman key that the
		//  Traffic Manager will accept when connecting using Diffie-Hellman
		//  key agreement as a client.
		Ssl3DiffieHellmanClientMinKeyLength *int `json:"ssl3_diffie_hellman_client_min_key_length,omitempty"`
