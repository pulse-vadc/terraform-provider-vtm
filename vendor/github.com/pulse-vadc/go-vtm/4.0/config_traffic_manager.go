...
func (vtm VirtualTrafficManager) GetTrafficManager(name string) (*TrafficManager, *vtmErrorResponse) {
	...
	conn := vtm.connector.getChildConnector("/tm/4.0/config/active/traffic_managers/" + name)
	data, ok := conn.get()
	if ok != true {
...
func (vtm VirtualTrafficManager) ListTrafficManagers() (*[]string, *vtmErrorResponse) {
	conn := vtm.connector.getChildConnector("/tm/4.0/config/active/traffic_managers")
	data, ok := conn.get()
	if ok != true {
		object := new(vtmErrorResponse)
		json.NewDecoder(data).Decode(object)
...
type TrafficManagerProperties struct {
	Appliance struct {
		...
		ShimClientId *string `json:"shim_client_id,omitempty"`

		// The client key provided by the portal for this server.
		ShimClientKey *string `json:"shim_client_key,omitempty"`

		// Enable the Riverbed Cloud SteelHead discovery agent on this appliance.
		ShimEnabled *bool `json:"shim_enabled,omitempty"`
		...
		//  last).
		ShimIps *string `json:"shim_ips,omitempty"`

		// The load balancing method for selecting a Riverbed Cloud SteelHead
		//  appliance.
		ShimLoadBalance *string `json:"shim_load_balance,omitempty"`

		// The minimum severity that the discovery agent will record to
		//  its log.
		ShimLogLevel *string `json:"shim_log_level,omitempty"`
	...
	Basic struct {
		...
		NumL4Children *int `json:"num_l4_children,omitempty"`

		// How many L7 worker processes should be created
		NumL7Children *int `json:"num_l7_children,omitempty"`
	...
	FaultTolerance struct {
		...
		LssDedicatedIps *[]string `json:"lss_dedicated_ips,omitempty"`
	...
	Iop struct {
		// Configure Loopback modes for performance testing at different
		//  levels. Refer to enum io_eal_loopback_config to understand loopback
		//  configuration
		EnableLb *int `json:"enable_lb,omitempty"`
		...
		L4EventDrivenMode *bool `json:"l4_event_driven_mode,omitempty"`

		// IOP Disable L7 Event Driven Mode
		L7EventDrivenMode *bool `json:"l7_event_driven_mode,omitempty"`
		...
		NumHugepages *int `json:"num_hugepages,omitempty"`

		// IOP Number of MBUFS per MPOOL
		NumMbufsPerMpool *int `json:"num_mbufs_per_mpool,omitempty"`

		// Traffic sent to Linux if set
		SendToLinux *bool `json:"send_to_linux,omitempty"`

		// Size of each Kernel's hugepage.
		SizeHugepage *string `json:"size_hugepage,omitempty"`
	} `json:"iop"`
